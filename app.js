...
/* ================= VIEW MODE (Expert / Light) ================= */
const VIEW_KEY  = "inj_view_mode"; // expert | essential
let viewMode = (localStorage.getItem(VIEW_KEY) || "expert").toLowerCase();

function applyViewMode(mode){
  viewMode = (mode === "essential") ? "essential" : "expert";
  document.body.classList.toggle("mode-essential", viewMode === "essential");
  try { localStorage.setItem(VIEW_KEY, viewMode); } catch {}

  const ic = $("viewIcon");
  if (ic) ic.textContent = (viewMode === "essential") ? "🔢" : "📊";
}
...
function refreshConnUI() {
  ...
  const loader = $("topLoader");
  const online = hasInternet();

  let state = "st-offline";
  let text = "Offline";
  ...
  statusDot.classList.remove("st-online", "st-loading", "st-offline");
  statusDot.classList.add(state);

  // top loader only when loading
  const isLoading = (state === "st-loading");
...
$("viewToggle")?.addEventListener("click", (e) => {
  e?.preventDefault?.();
  applyViewMode(viewMode === "essential" ? "expert" : "essential");
}, { passive:false });
...
function drawNW(force=false){
  ...
  if (netWorthChart && $("nwReadout")) {
    ...
    if (ds.length) {
      ...
      $("nwReadout").textContent = `${ts ? fmtHHMM(ts) : "—"} • $${safe(ds[i]).toFixed(2)}`;
    }
  }
}
...
/* ================= DASHBOARD PREFERENCES (card order) ================= */
const ORDER_KEY = "inj_dash_order_v1";

function getCardNodes(){
  const wrap = $("cardsWrapper") || document.querySelector(".cards-wrapper");
  if (!wrap) return { wrap:null, cards:[] };
  const cards = Array.from(wrap.querySelectorAll(".card[data-card]"));
  return { wrap, cards };
}
function getDefaultOrder(){
  const { cards } = getCardNodes();
  return cards.map(c => c.dataset.card).filter(Boolean);
}
...
function applyCardOrder(order){
  ...
  for (const c of cards){
    ...
    if (!used.has(key)) wrap.appendChild(c);
  }
}

function renderOrderList(){
  const host = $("dashOrderList");
  if (!host) return;

  const { cards } = getCardNodes();
  const order = loadCardOrder() || getDefaultOrder();
  const titleMap = new Map(cards.map(c => [c.dataset.card, c.dataset.title || c.querySelector(".label")?.textContent || c.dataset.card]));

  host.innerHTML = "";
  const items = order.map(k => ({ key:k, title: titleMap.get(k) || k }))
    .filter(x => x.key);
  ...
  for (const it of items){
    ...
    row.innerHTML = `<span class="drag">⠿</span><span class="name">${it.title}</span>`;
    host.appendChild(row);
  }

  // mobile-friendly buttons (up/down)
  if (!host.dataset.boundClick){
    ...
    host.addEventListener("click", (e) => {
      ...
      const ord = loadCardOrder() || getDefaultOrder();
      const i = ord.indexOf(key);
      if (i < 0) return;

      if (act === "up" && i > 0){
        [ord[i-1], ord[i]] = [ord[i], ord[i-1]];
      } else if (act === "down" && i < ord.length - 1){
        [ord[i+1], ord[i]] = [ord[i], ord[i+1]];
      } else {
        return;
      }
...
function readOrderList(){
  const host = $("dashOrderList");
  if (!host) return getDefaultOrder();
  return Array.from(host.querySelectorAll(".order-item")).map(x => x.dataset.key).filter(Boolean);
}

function bindPreferencesUI(){
  // tabs
  const tabApp = $("settingsTabApp");
  const tabPrefs = $("settingsTabPrefs");
  const paneApp = $("settingsPaneApp");
  const panePrefs = $("settingsPanePrefs");
  if (tabApp && tabPrefs && paneApp && panePrefs){
    const setTab = (which) => {
      const prefs = (which === "prefs");
      tabApp.classList.toggle("active", !prefs);
      tabPrefs.classList.toggle("active", prefs);
      paneApp.classList.toggle("active", !prefs);
      panePrefs.classList.toggle("active", prefs);
      if (prefs) renderOrderList();
    };
    tabApp.addEventListener("click", () => setTab("app"), { passive:true });
    tabPrefs.addEventListener("click", () => setTab("prefs"), { passive:true });
  }
  ...
  $("dashOrderApply")?.addEventListener("click", () => {
    ...
    pushEvent({ kind:"info", title:"Preferences applied", detail:"Dashboard order saved.", status:"done" });
  }, { passive:true });
  ...
  $("dashOrderReset")?.addEventListener("click", () => {
    ...
    applyCardOrder(order);
    renderOrderList();
    pushEvent({ kind:"info", title:"Preferences reset", detail:"Dashboard order restored.", status:"done" });
  }, { passive:true });
}
...
(async function boot() {
  ...
  bindExpandButtons();
  bindPreferencesUI();
  applyCardOrder(loadCardOrder() || getDefaultOrder());
  setAddressDisplay(address);
...
function animate() {
  ...
  // Rewards Earned (total withdrawn per wallet)
  const earned = Array.isArray(wdValuesAll) ? wdValuesAll.reduce((a,v)=>a+safe(v),0) : 0;
  setText("rewardsEarned", earned.toFixed(6));
  setText("rewardsEarnedUsd", `≈ $${(earned * displayed.price).toFixed(2)}`);
