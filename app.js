...
function openDrawer(){
  ...
  try{ if (menuBtn) menuBtn.textContent = "✕"; } catch {}
}
...
drawer?.addEventListener("click", (e) => {
  const el = e?.target;
  if (!el) return;
  // Any click on a button inside drawer that is not a toggle should close
  const btn = el.closest?.("button");
  if (!btn) return;
  if (btn.classList.contains("toggle-btn")) return;
  if (btn.id === "themeToggle" || btn.id === "privacyToggle" || btn.id === "liveToggle" || btn.id === "viewToggle") return;
  if (btn.classList.contains("nav-item")) return; // handled above
  // e.g., settings buttons inside
