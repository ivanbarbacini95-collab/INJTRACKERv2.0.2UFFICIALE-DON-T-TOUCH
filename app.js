...
/* ================= ICONS (SVG, ASCII-safe) ================= */
const ICONS = {
  moon: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8Z"/></svg>',
  sun: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 18a6 6 0 1 0 0-12 6 6 0 0 0 0 12Z"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/></svg>',
  antenna: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 21v-4"/><path d="M8 17a4 4 0 0 1 8 0"/><path d="M5 17a7 7 0 0 1 14 0"/><path d="M12 3a6 6 0 0 1 6 6"/></svg>',
  refresh: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M21 12a9 9 0 0 1-15.4 6.4"/><path d="M3 12a9 9 0 0 1 15.4-6.4"/><path d="M3 18v-6h6"/><path d="M21 6v6h-6"/></svg>',
  grid: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3h8v8H3zM13 3h8v8h-8zM3 13h8v8H3zM13 13h8v8h-8z"/></svg>',
  eye: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s4-7 10-7 10 7 10 7-4 7-10 7-10-7-10-7Z"/><path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"/></svg>',
  eyeOff: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3l18 18"/><path d="M10.6 10.6a2 2 0 0 0 2.8 2.8"/><path d="M9.9 5.1A10.7 10.7 0 0 1 12 5c6 0 10 7 10 7a17 17 0 0 1-3.1 4.2"/><path d="M6.1 6.1A17.3 17.3 0 0 0 2 12s4 7 10 7a10.3 10.3 0 0 0 4.1-.9"/></svg>',
  bolt: '<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M13 2 3 14h8l-1 8 10-12h-8l1-8Z"/></svg>'
};
...
/* ================= ONCE BIND HELPERS ================= */
const __injOnce = (window.__injOnce = window.__injOnce || Object.create(null));
function docOnce(key, type, handler, opts){
  if (__injOnce[key]) return;
  __injOnce[key] = 1;
  document.addEventListener(type, handler, opts);
}
function bindOnce(el, flag, type, handler, opts){
  if (!el) return;
  try{
...
function applyTheme(t){
  ...
  setIcon("themeIcon", theme === "dark" ? ICONS.moon : ICONS.sun);
  refreshChartsTheme();
...
function applyView(v){
  ...
  if (icon) icon.innerHTML = (viewMode === "lite") ? ICONS.bolt : ICONS.grid;
...
function updatePerf(arrowId, pctId, v) {
  ...
  else if (v < 0) { arrow.textContent = "v"; arrow.className = "arrow down"; pct.className = "pct down"; }
...
bindOnce(searchBtn,"boundSearch","click",(e)=>{
  e?.preventDefault?.();
  e?.stopPropagation?.();
...
docOnce("doc_click_search","click",(e)=>{
  if (!searchWrap) return;
...
bindOnce(menuBtn,"boundMenu","click",(e)=>{
  e?.preventDefault?.();
  e?.stopPropagation?.();
  toggleDrawer();
}, { passive: false });

bindOnce(backdrop,"boundBackdrop","click",()=>closeDrawer(),{passive:true});

docOnce("doc_keydown_escape","keydown",(e)=>{
  if (e.key === "Escape") {
    closeDrawer();
    closeComingSoon();
    exitFullscreenCard();
...
function setMode(isLive){
  ...
  setIcon("liveIcon", liveMode ? ICONS.antenna : ICONS.refresh);
...
function animateCountText(el, from, to, ms, fmt, tick){
  if (!el) return;
...
function updateTotalRewardAccUI(){
  ...
  const fmtInj = (v) => `+${Number(v).toFixed(6)}`;
  const fmtUsd = (v) => px ? `~ $${(Number(v) * px).toFixed(4)}` : "~ $-";

  if (!changed){
    // ensure visible after first paint
    if (out && !out.textContent) out.textContent = fmtInj(total);
    if (usd && !usd.textContent) usd.textContent = fmtUsd(total);
    return;
  }

  // increase -> count up in green + flash card
  if (total > prev){
    if (card){
    ...
    if (out){
      out.classList.add("count-green");
      animateCountText(out, prev, total, 700, fmtInj, (_,p)=>{ if(p>=1) out.classList.remove("count-green"); });
    }
    if (usd){
      usd.classList.add("count-green");
      animateCountText(usd, prev, total, 700, fmtUsd, (_,p)=>{ if(p>=1) usd.classList.remove("count-green"); });
    }
  } else {
    // decrease -> immediate
    if (out) out.textContent = fmtInj(total);
    if (usd) usd.textContent = fmtUsd(total);
  }
...
let backupMeta = { localAt: 0, cloudAt: 0, cloudState: "IDLE", cloudErr: "" };
...
function backupLoadMeta(){
  ...
  if (!k) { backupMeta = { localAt: 0, cloudAt: 0, cloudState:"-", cloudErr:"" }; return; }
  try{
    const raw = localStorage.getItem(k);
    if (!raw) { backupMeta = { localAt: 0, cloudAt: 0, cloudState:"-", cloudErr:"" }; return; }
    const o = JSON.parse(raw);
    backupMeta = {
      localAt: Number(o?.localAt || 0),
      cloudAt: Number(o?.cloudAt || 0),
      cloudState: String(o?.cloudState || "-").replace(/[^\x20-\x7E]/g,"").slice(0,24) || "-",
      cloudErr: String(o?.cloudErr || "").replace(/[^\x20-\x7E]/g,"").slice(0,160)
    };
  }catch{
    backupMeta = { localAt: 0, cloudAt: 0, cloudState:"-", cloudErr:"" };
  }
}
...
function backupSaveLocalSnapshot(){
  ...
  if (!k) return;
  const payload = { v: BACKUP_VER, address, t: Date.now(), payload: buildCloudPayload() };
...
async function backupPullLatest(){
  ...
  // Apply snapshot (merge + dedup)
  try{
    mergeStakeByLabel(payload.stake);
    mergeWdAll(payload.wd);
    mergeNWAll(payload.nw);
    mergeEventsAll(payload.events);
    ...
    backupMeta.cloudAt = Number(snap.t || Date.now());
    backupMeta.cloudState = "SYNCED";
    backupMeta.cloudErr = "";
    backupSaveMeta();
    backupRenderUI();

    // also store local snapshot so browser has it offline
    backupSaveLocalSnapshot();

    return snap;
  } catch (e){
    backupMeta.cloudState = "ERROR";
    backupMeta.cloudErr = "apply failed";
    backupSaveMeta();
    backupRenderUI();
    return null;
  }
...
async function cloudPull(){
  ...
  try{
    ...
    //  merge solo del tuo address, payload e per-address gia dal server
...
async function commitAddress(newAddr) {
  ...
  loadAprLocal();
  drawAprChart();
  // backup pull (per-wallet snapshot)
  try { await backupPullLatest(); } catch {}
...
function applyPrivacy(on){
  ...
  if (ico) ico.innerHTML = privacyOn ? ICONS.eyeOff : ICONS.eye;
}
...
// Call boot immediately (defer scripts run after DOM parse). Fallback on DOMContentLoaded just in case.
__bootOnce();
document.addEventListener("DOMContentLoaded", __bootOnce, { once:true });
