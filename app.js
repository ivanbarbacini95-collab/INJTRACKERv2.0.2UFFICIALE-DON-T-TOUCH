...
/* ================= LOOP ERROR THROTTLE ================= */
let __loopErrLastAt = 0;
let __loopErrLastMsg = "";
let __loopErrCount = 0;
let __disableChartsUpdates = false;
function __reportLoopError(e){
  const msg = (e && (e.message || e.toString())) ? String(e.message || e.toString()) : "unknown";
  const now = Date.now();
  if (msg !== __loopErrLastMsg || (now - __loopErrLastAt) > 2000){
    __loopErrLastMsg = msg;
    __loopErrLastAt = now;
    __loopErrCount = 0;
    setStatusError("Loop Error: " + msg);
  } else {
    __loopErrCount++;
    // after repeated identical errors, stop chart updates to keep UI alive
    if (__loopErrCount > 20) __disableChartsUpdates = true;
  }
}
/* ================= THEME / MODE ================= */
...
function refreshConnUI(){
  ...
  dot.classList.remove("is-online","is-loading","is-offline");
...
function openSearch(){
  const dlg = $("searchModal");
  const input = $("addrInput");
  if(!dlg || !input) return;

  // per richiesta: la barra rimane vuota dopo una ricerca,
  // ma l’address resta memorizzato (auto-load al prossimo avvio)
  input.value = "";
  input.focus();
...
function updateChartFrom1mKline(k) {
  ...
  if (lastChartMinuteStart === openTime) {
    ...
    if (idx >= 0) {
      ...
      try{ chart.update("none"); } catch(e){ __reportLoopError(e); }
    }
    return;
  }
...
function syncRewardTimelineUI(forceToEnd=false) {
  ...
  if (!n) {
    ...
    if (rewardChart) {
      ...
      try{ rewardChart.update("none"); } catch(e){ __reportLoopError(e); }
    }
    return;
  }
...
function refreshChartsTheme(){
  try{
    if (stakeChart) {
      ...
      try{ stakeChart.update("none"); } catch(e){ __reportLoopError(e); }
    }
    if (rewardChart) {
    ...
    if (netWorthChart) {
      ...
      try{ netWorthChart.update("none"); } catch(e){ __reportLoopError(e); }
    }
  } catch {}
}
...
/* === View Mode (Lite / Pro) === */
const VIEW_KEY = "inj_view_mode_v1";
let viewMode = localStorage.getItem(VIEW_KEY) || "pro"; // "pro" | "lite"

function applyViewMode(mode){
  viewMode = (mode === "lite") ? "lite" : "pro";
  localStorage.setItem(VIEW_KEY, viewMode);

  document.body.classList.toggle("mode-essential", viewMode === "lite");

  const ico = $("viewIcon");
  const hint = $("viewHint");
  if(ico) ico.textContent = (viewMode === "lite") ? "🪶" : "🚀";
  if(hint) hint.textContent = (viewMode === "lite") ? "Lite" : "Pro";
}

$("viewToggle")?.addEventListener("click", (e)=>{
  e?.preventDefault?.();
  applyViewMode(viewMode === "lite" ? "pro" : "lite");
}, { passive:false });
...
function animate() {
  ...
  if (!__disableChartsUpdates){
    try { drawNW(false); } catch (e) { __reportLoopError(e); }
    try { updateNetWorthMiniRows(); } catch (e) { __reportLoopError(e); }
  }

  if (!__disableChartsUpdates){
    try { if (address && liveMode) recordNetWorthPoint(); } catch (e) { __reportLoopError(e); }
  }
...
(function(){
  ...
  const items = getCardsInCurrentOrder().map(c => ({
    id: c.id,
    title: (c.querySelector(".label")?.textContent || c.id || "Card").trim()
  }));
  ...
  for(const it of items){
    ...
    li.draggable = true;
    li.dataset.cardId = it.id;

    const handle = document.createElement("span");
    handle.className = "pref-handle";
    handle.textContent = "⋮⋮";
    ...
    actions.className = "pref-actions";

    const up = document.createElement("button");
    up.type = "button";
    up.className = "mini-btn pref-move";
    up.textContent = "↑";
    ...
    down.type = "button";
    down.className = "mini-btn pref-move";
    down.textContent = "↓";
    ...
    li.addEventListener("dragover", (e)=>{
      e.preventDefault();
      const dragging = list.querySelector(".pref-item.dragging");
    ...
    // Touch drag (mobile) — keep existing behavior but easier with ↑/↓
