...
function applyViewMode(mode){
  ...
  // Update Settings snapshot if present
...
function startTradeWS() {
  ...
  wsTrade.onmessage = (e) => {
    ...
    if (tfReady.y) { candle.y.high = Math.max(candle.y.high, p); candle.y.low = Math.min(candle.y.low, p); }
  };
}
...
async function loadCandleSnapshot(isRefresh=false) {
  ...
  const [d, w, m, y] = await Promise.all([
    ...
    fetchJSON("https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=1M&limit=12"),
  ]);
  ...
  // 1Y derived from the last 12 monthly candles
...
function animate() {
  ...
  const yUp   = "linear-gradient(90deg, rgba(168,85,247,.45), rgba(59,130,246,.25))";
  const yDown = "linear-gradient(270deg, rgba(239,68,68,.38), rgba(168,85,247,.22))";
