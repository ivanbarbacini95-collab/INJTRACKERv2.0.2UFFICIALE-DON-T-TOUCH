...
/* Cloud API base override (useful when UI runs on GitHub Pages but API is on Vercel) */
const CLOUD_API_BASE_VER = 1;
const CLOUD_API_BASE_KEY = `inj_cloud_api_base_v${CLOUD_API_BASE_VER}`;

function getCloudApiBase(){
  try { return String(localStorage.getItem(CLOUD_API_BASE_KEY) || "").trim(); } catch { return ""; }
}
function setCloudApiBase(v){
  try { localStorage.setItem(CLOUD_API_BASE_KEY, String(v || "").trim()); } catch {}
}
function cloudApiUrl(){
  const base = getCloudApiBase();
  if (!base) return CLOUD_API;
  const b = base.replace(/\/+$/, "");
  if (b.endsWith("/api/point")) return b;
  return b + CLOUD_API;
}

/* Account Sync (per-address): when ON, cloud pull/push is automatic */
const ACC_SYNC_VER = 1;
function accSyncKey(addr){
  const a = String(addr || "").trim();
  return a ? `inj_accsync_v${ACC_SYNC_VER}_${a}` : null;
}
function loadAccSync(addr){
  const k = accSyncKey(addr);
  if (!k) return false;
  try { return localStorage.getItem(k) === "1"; } catch { return false; }
}
function saveAccSync(addr, on){
  const k = accSyncKey(addr);
  if (!k) return;
  try { localStorage.setItem(k, on ? "1" : "0"); } catch {}
}
let accSyncEnabled = false;

const BACKUP_LOCAL_VER = 1;
function backupLocalKey(addr){
  const a = String(addr || "").trim();
  return a ? `inj_backup_local_last_v${BACKUP_LOCAL_VER}_${a}` : null;
}
function loadBackupLocalLast(addr){
  const k = backupLocalKey(addr);
  if (!k) return 0;
  try { return safe(localStorage.getItem(k)); } catch { return 0; }
}
function saveBackupLocalLast(addr, ts){
  const k = backupLocalKey(addr);
  if (!k) return;
  try { localStorage.setItem(k, String(Math.floor(safe(ts)))); } catch {}
}
function touchBackupLocal(){
  if (!address) return;
  saveBackupLocalLast(address, Date.now());
  try { renderBackupSettingsUI(); } catch {}
}
...
function maybeRecordRewardWithdrawal(newRewards) {
  ...
  if (diff > REWARD_WITHDRAW_THRESHOLD) {
    ...
    // ✅ Update Total Reward Accumulate (flash green + "+")
    try { updateTotalRewardAccUI({ flash:true }); } catch {}
...
function updateTotalRewardAccUI(opts = {}){
  ...
  // animate only when the INJ total changes (not on price ticks)
  if (out){
    if (_trAccLast == null) {
      out.textContent = `+${total.toFixed(6)}`;
    } else if (Math.abs(total - _trAccLast) > 1e-9 && opts.flash) {
      _animNumber(out, _trAccLast, total, 750, 6);
    } else {
      out.textContent = `+${total.toFixed(6)}`;
    }
  }
  ...
  if (usd){
    const v = total * px;
    usd.textContent = Number.isFinite(v) ? `≈ +$${v.toFixed(2)}` : "≈ +$0.00";
  }

  if (_trAccLast != null && Math.abs(total - _trAccLast) > 1e-9 && opts.flash && card){
...
function cloudSetState(state){
  const _paused = (!!address && !accSyncEnabled && state !== "saving" && state !== "error");
  ...
  if (cloudDotMenu) {
    ...
    if (_paused) cloudDotMenu.classList.add("off");
  ...
  if (cloudTextMenu) {
    cloudTextMenu.textContent = _paused ? "Paused"
      : (state === "saving") ? "Saving"
      : (state === "error") ? "Error"
  ...
  if (advDot) {
    ...
    if (_paused) advDot.classList.add("off");
...
function cloudMarkDirty(what = {}){
  ...
  if (!accSyncEnabled) { cloudSetState("synced"); return; }
  if (!hasInternet()) return;
  scheduleCloudPush();
}
function scheduleCloudPush(){
  if (!accSyncEnabled) return;
...
function mergeNW(payloadNw){
  ...
  // ✅ avoid duplicates across devices by bucketing timestamps
  const BUCKET = 15_000; // 15s buckets keep the curve smooth but remove "double points"

  const map = new Map();
  const add = (rawT, usdV, injV) => {
    const rt = safe(rawT);
    if (!rt) return;
    const key = Math.floor(rt / BUCKET) * BUCKET;
    const prev = map.get(key);
    if (!prev || rt > prev.raw) map.set(key, { raw: rt, usd: safe(usdV), inj: safe(injV) });
  };

  for (let i=0;i<nwTAll.length;i++){
    add(nwTAll[i], nwUsdAll[i], nwInjAll[i]);
  }
  ...
  nwTAll = keys;
  nwUsdAll = keys.map(k => map.get(k).usd);
  nwInjAll = keys.map(k => map.get(k).inj);
...
async function cloudPull(){
  ...
  const url = `${cloudApiUrl()}?address=${encodeURIComponent(address)}`;
...
/* ================= BACKUP + ACCOUNT SYNC (Settings) ================= */
function renderBackupSettingsUI(){
  const apiIn = $("cloudApiBaseInput");
  if (apiIn && apiIn.value.trim() !== getCloudApiBase()) apiIn.value = getCloudApiBase();

  const localLast = $("backupLocalLast");
  const cloudLast = $("backupCloudLast");
  const cloudState = $("backupCloudState");
  const dot = $("backupCloudDot");
  const note = $("backupNote");
  const accState = $("accSyncState");
  const accT = $("accSyncToggle");

  const localTs = address ? loadBackupLocalLast(address) : 0;
  if (localLast) localLast.textContent = localTs ? new Date(localTs).toLocaleString() : "—";
  if (cloudLast) cloudLast.textContent = cloudLastSync ? new Date(cloudLastSync).toLocaleString() : "—";

  const paused = (!!address && !accSyncEnabled);
  const st = !address ? "—" : paused ? "Paused" : (hasInternet() ? "Synced" : "Offline cache");
  if (cloudState) cloudState.textContent = st;

  if (dot){
    dot.classList.remove("ok","saving","err","off");
    if (!address) dot.classList.add("off");
    else if (paused) dot.classList.add("off");
    else dot.classList.add(hasInternet() ? "ok" : "off");
  }

  if (accT) accT.checked = !!accSyncEnabled;
  if (accState) accState.textContent = accSyncEnabled ? "ON" : "OFF";

  if (note){
    const base = getCloudApiBase();
    note.textContent = base ? `Cloud API: ${base}` : "Cloud API: (same domain)";
  }
}

$("cloudApiBaseSaveBtn")?.addEventListener("click", (e) => {
  e.preventDefault();
  const v = $("cloudApiBaseInput")?.value?.trim() || "";
  setCloudApiBase(v);
  pushEvent({ kind:"info", title:"Cloud API base saved", detail: v ? v : "(same domain)", status:"ok" });
  renderBackupSettingsUI();
}, { passive:false });

$("backupNowBtn")?.addEventListener("click", async (e) => {
  e.preventDefault();
  if (!address) { pushEvent({ kind:"info", title:"Backup", detail:"Insert an address first.", status:"fail" }); return; }
  if (!hasInternet()) { pushEvent({ kind:"info", title:"Backup", detail:"Offline: cannot push to cloud.", status:"fail" }); return; }
  ...
  try{
    await cloudPush();
    pushEvent({ kind:"info", title:"Backup complete", detail:"Cloud synced", status:"ok" });
  } catch {
    pushEvent({ kind:"info", title:"Backup failed", detail:"Cloud error", status:"fail" });
  }
  renderBackupSettingsUI();
}, { passive:false });

$("accSyncToggle")?.addEventListener("change", (e) => {
  const on = !!e.target.checked;
  accSyncEnabled = on;
  if (address) saveAccSync(address, on);
  renderBackupSettingsUI();
  cloudSetState("synced");

  if (on && address && hasInternet()){
    cloudSetState("saving");
...
async function commitAddress(newAddr) {
  ...
  loadAprLocal();
  drawAprChart();
// Load Account Sync preference for this address
accSyncEnabled = loadAccSync(address);
try { renderBackupSettingsUI(); } catch {}

// cloud pull only if Account Sync is ON
if (accSyncEnabled) {
  cloudSetState("saving");
  safeAsync(() => cloudPull(), "cloudPull");
} else {
