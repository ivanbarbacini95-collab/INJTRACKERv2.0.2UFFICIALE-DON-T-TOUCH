...
window.addEventListener("online", () => {
  ...
  if (liveMode){
    ...
    // (FINAL) loadChartToday disabled (use loadPriceChart)
...
function applyView(v){
  ...
  // ✅ When switching from LITE → PRO, charts were previously hidden and could render at 0px.
  // Re-init / resize them on the next frame and refresh data once.
  if (viewMode === "pro"){
    requestAnimationFrame(() => {
      try { if (!stakeChart) initStakeChart(); else stakeChart.resize?.(); } catch {}
      try { if (!rewardChart) initRewardWdChart(); else rewardChart.resize?.(); } catch {}
      try { if (!aprChart) initAprChart(); else aprChart.resize?.(); } catch {}
      try { if (!netWorthChart) initNWChart(); else netWorthChart.resize?.(); } catch {}
      try { if (!chart) initChartToday(); } catch {}

      if (document.body?.dataset?.view !== "lite") { try { applyPriceCache(priceTf); } catch {} }
      safeAsync(() => loadPriceChart(true), "loadPriceChart(view-pro)");

      try { drawNW(true); } catch {}
      try { drawStakeChart(true); } catch {}
      try { drawRewardWdChart(true); } catch {}
      try { drawAprChart(true); } catch {}
    ...
    // Save bandwidth / stop in-flight chart fetch during fast switching
...
function startAllTimers(){
  ...
  chartSyncTimer = setInterval(() => safeAsync(() => loadPriceChart(false), "loadPriceChart(timer)"), CHART_SYNC_MS);
  // (FINAL) ensureChartTimer disabled (reduces CPU / avoids missing fn)
...
function drawStakeChart() {
  if (document.body?.dataset?.view === "lite") return;
...
function applyPriceCache(tf){
  const canRender = (document.body?.dataset?.view !== "lite");
...
async function loadPriceChart(force=false){
  ...
  const canRender = (document.body?.dataset?.view !== "lite");
  if (canRender && !chart) initChartToday();
...
safeAsync(() => cloudPull(), "cloudPull");
  ...
  // (FINAL) loadChartToday removed: unified price chart pipeline uses loadPriceChart()
